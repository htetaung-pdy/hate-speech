...
# Constants
EXT = 'csv'
OUTPUT_PATH = '/home/bupi/Documents/pdy/hs/dashboard/clean/aggregated/{}'.format(datetime_suffix())
...
def run(comment_files):
    ...
    date_to = comment_files[-1].split('/')[-1].rstrip('.csv').split('_')[-1]
...
def load_df(comment_file, date_from, date_to):
    ...
    post_df = pd.read_csv(post_file, usecols=['page_name','post_url', 'lex_found'])
    # join
...
def lex_topic_page_time(comment_wlex):
    proflat, dtflat, lexflat, topicsflat, hours, pages = [], [], [], [], [], []
    for pid, dt, lex, topics, page in zip(
        comment_wlex.profile_id, comment_wlex.datetime, comment_wlex.lex_found,
        comment_wlex.topics_found, comment_wlex.page_name):
        lex = lex.split('~')
...
def page_reach(comment_df):
    page_reach_sr = comment_df.groupby(['page_name','date'])['profile_id'].nunique()
